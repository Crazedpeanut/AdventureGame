class Assets {
    ...
    loadTileMap(tileMapUrl) {
        const preLoadedTilemap = this.tileMaps[tileMapUrl];

        if(preLoadedTilemap) {
            return Promise.resolve(preLoadedTilemap);
        }

        return this.loadAsset(tileMapUrl)
            .then(image => {
                this.tileMaps[tileMapUrl] = image;
                return image;
            });
    }
    ...
    loadTileFromTileMap(tileMapUrl, topLeftCoords, bottomRightCoords) {
        return this.loadTileMap(tileMapUrl);
    }
}
